...
| Limits and pagination | `PaginationSafeguardFinderTest` (8): `maxPages`, `pageSize` and `inMemoryRowCeiling` on Query, Scan, PartiQL, `count` and in-memory ordering. Plus `FinderDrivenPaginationLoadTest` (3), where the **default** config pages because the result exceeds DynamoDB's 1 MB cap: ~2 MB across 80 versions of one key, and the same through a 50-key IN fan-out | The 1 MB cap is exercised; **scale is not** — no throttling, no hot-partition pressure, no concurrency, largest table is tens of MB |
